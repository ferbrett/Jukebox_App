...
import playlist.Student;
import playlist.StudentList;
...
public class StudentTest {
	...
	//Test the list of student names and ensure we get back the name value we expect at the correct/specific point in the list
		...
		@Test
		public void testGetJustinPerault() {
			List<String> studentNames = new ArrayList<String>();							//create variable for student list of names
			StudentList studentList = new StudentList();									//instantiate the StudentList object so we can access it's methods and properties
			studentNames = studentList.getStudentsNames();									//populate the studentNames list with the actual values in the StudentsList object
			assertEquals("Justin Perault", studentNames.get(2));								//test case to see if the third value contains the name we expect
		}
		
		//Test the list of student names and ensure we get back Angela Ruhstorfer
		@Test
		public void testGetAngelaRuhstorfer() {
			List<String> studentNames = new ArrayList<String>();							//create variable for student list of names
			StudentList studentList = new StudentList();									//instantiate the StudentList object so we can access it's methods and properties
			studentNames = studentList.getStudentsNames();									//populate the studentNames list with the actual values in the StudentsList object
			assertEquals("Angela Ruhstorfer", studentNames.get(3));							//test case for pass/fail. We expect the name to be Angela Ruhstorfer. Remember arrays start their count at 0 not 1.
		}
